...
<script>
    $(function () {
        var response = "{{request()->segment(2)}}"
        ...
        } else if (type === "sendwyre") {
            if (response === "success") {
                let getData = url.split("?")
                let getData2 = getData[1].split("&")
                let orderId = getData2[0].replace('orderId=', "")
                let uuid = getData2[1].replace('uuid=', "")
                $.ajax({
                    url: window.origin + "/api/v1/sendwyre-check-order-status",
                    method: "post",
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    data: {"orderId": orderId, "uuid": uuid},
                    success: function (res) {
                        console.log(res)
                    },
                    error: function (err) {

...
//        dd($request->all());
        try {
            ...
            $todayList = OnDemand::where('date', $request->date)->first();
            if ($todayList) {
                return response([
                    "status" => "error",
...
//                        dd($request->audio['link'][$index]);
                ...
                if (request()->has('audio_title')){
                    ...
                    if (!request()->has('audio_file')) {
                        return response([
                            "status" => "error",
                            "message" => "Sorry you did not select file"
                        ], 422);
                    }
...
//            dd($audioArray);
                ...
                if ($playlist->save()) {
                    return response([
                        "status" => "success",
                        "message" => "Podcast Successfully Create"
                    ], 200);
                }
            }
...
//        dd($request->all());
        try {
            ...
            $exist = OnDemand::where("id", "!=", $id)->where("date", $request->date)->first();


            if (!$exist) {
                $playlist = OnDemand::where("id", $id)->first();
                ...
                if ($playlist->update()) {
                    ...
                    return response([
                        "status" => "success",
                        "message" => "Podcast Successfully Update"
                    ], 200);
                }
            }else{
                return response([
    ...
    {
        $playlist = OnDemand::orderBy('id', 'desc')->paginate(20);
        return response([
            "status" => "success",
            "data" => $playlist
        ]);
    ...
    public function getPlaylistSearchData(Request $request)
    {
        try {
            if ($request->value === "today") {
                $getAllContest = OnDemand::whereDate('created_at', today())->paginate(20);
                if ($getAllContest) {
                    return response([
                        "status" => "success",
                        "data" => $getAllContest
                ...
                $getAllContest = OnDemand::whereDate('created_at', today()->subDay())->paginate(20);
                if ($getAllContest) {
                    return response([
                        "status" => "success",
                        "data" => $getAllContest
                ...
                $getAllContest = OnDemand::whereDate('created_at', today()->subDays(6))->paginate(20);
                if ($getAllContest) {
                    return response([
                        "status" => "success",
                        "data" => $getAllContest
                ...
                $getAllContest = OnDemand::whereMonth('created_at', today()->subMonth())->paginate(20);
                if ($getAllContest) {
                    return response([
                        "status" => "success",
                        "data" => $getAllContest
                ...
                $getAllContest = OnDemand::whereYear('created_at', today()->subYear())->paginate(20);
                if ($getAllContest) {
                    return response([
                        "status" => "success",
                        "data" => $getAllContest
...
//        dd($request->all());
        $audioArray = [];
        $playlist = OnDemand::where("id", $request->contestId)->first();
